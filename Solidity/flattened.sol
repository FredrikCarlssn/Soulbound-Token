...
abstract contract SoulboundCore is Context, ERC165, IERC721Errors {
...
contract SoulboundToken is SoulboundCore, SoulboundStorage {
    ...
    constructor() SoulboundCore("SoulBound", "SBT") {}
    ...
    function mintSoulboundToken(
        ...
        if (mintingType == uint8(MintingType.Everyone)) {
            if (claimedSoulboundToken[recipient][_soulboundToken] == true) {
                revert invalidMint(mintingType, _soulboundToken, recipient);
            } else {
                claimedSoulboundToken[recipient][_soulboundToken] = true;
            }
        }

        if (mintingType == uint8(MintingType.AllowList)) {
            if (
                allowList[recipient][_soulboundToken] == false ||
                recipient != sender ||
                claimedSoulboundToken[recipient][_soulboundToken] == true
            ) {
                revert invalidMint(mintingType, _soulboundToken, recipient);
        ...
        if (mintingType == uint8(MintingType.Owner)) {
            if (owner() != sender) {
                revert invalidMint(mintingType, _soulboundToken, recipient);
            } else {
                claimedSoulboundToken[recipient][_soulboundToken] = true;
